...
В ходе работы были реализованы три функции:

- **`count_combo_task1()`**  
  Использует `itertools.product` для генерации всех 5-буквенных комбинаций из букв {'Г','Е','П','А','Р','Д'}. Проверяет условия: количество букв 'Г' равно 1, первый символ не 'А', последний не 'Е'. Возвращает количество подходящих слов.

- **`count_combo_task2()`**  
  Вычисляет значение \(5^{36} + 5^{24} - 25\) и переводит его в пятеричную систему счисления путём последовательного деления на 5. Подсчитывает количество цифр, равных 4.

- **`find_numbers_with_divisor_ending_8()`**  
  Перебирает целые числа, начиная с 500001, и для каждого находит все делители (перебор до \(\sqrt{n}\)). Из делителей, оканчивающихся на 8, исключает 8 и само число, выбирает минимальный. Останавливается, когда найдено 5 таких чисел.

Для всех функций написаны доктесты, проверяющие:
- корректность результатов,
- типы возвращаемых данных,
- выполнение условий задачи (например, что все делители оканчиваются на 8, числа упорядочены и т.д.).
