...
тройная вложенность:
```python
@cache_decorator(maxsize=5, logging_enabled=True)
def my_func():
    pass
```
Это превращается в:
- cache_decorator(maxsize=5, logging_enabled=True) → возвращает decorator
- @decorator → вызывает decorator(my_func) → возвращает wrapper
- Теперь my_func = wrapper
- 
Внутренности декоратора
```python
def decorator(func):
    memory = {}              # Хранилище кэша
    cache_hits = 0           # Счётчик попаданий
    cache_misses = 0         # Счётчик промахов
    recursion_depth = 0      # Для рекурсии
    
    @wraps(func)             # Сохраняет имя и докстринг функции
...
1. memory = {} - словарь, где ключ = аргументы функции, значение = результат
2. cache_hits - сколько раз результат брался из кэша (быстро)
3. cache_misses - сколько раз пришлось вычислять (медленно)
4. recursion_depth - глубина рекурсии (для отступов в логах)

Создание ключа для кэша
```python
key = (args, tuple(sorted(kwargs.items())))
```
Почему sorted(kwargs.items())?
- Чтобы func(a=1, b=2) и func(b=2, a=1) давали одинаковый ключ
- Именованные аргументы могут быть в любом порядке, но результат один
...
if key in memory:
    ...
    return memory[key]  # Быстро возвращаем результат
```
Вычисление и сохранение
```python
cache_misses += 1
result = func(*args, **kwargs)  # Реально вычисляем
memory[key] = result             # Сохраняем в кэш
return result
```
LRU (Least Recently Used) - ограничение кэша
```python
if maxsize is not None and len(memory) > maxsize:
    oldest_key = next(iter(memory))  # Берём первый ключ
    del memory[oldest_key]           # Удаляем его
```
Как это работает:
- Python 3.7+ сохраняет порядок добавления ключей в словаре
- next(iter(memory)) даёт самый старый ключ
- Когда кэш переполняется, удаляется давно неиспользуемый элемент

Поддержка рекурсии
```python
recursion_depth += 1
indent = "  " * (recursion_depth - 1)  # Отступ для красоты

# ... вызов функции (которая может вызвать саму себя)

recursion_depth -= 1  # При выходе уменьшаем глубину
```
Логирование
```python
if logging_enabled:
...
- Откуда взят результат (из кэша или вычислен)

Добавление методов к wrapper
```python
def cache_info():
    return {
        'size': len(memory),
...
Поддержка разных способов вызова
```python
if callable(maxsize):
    # Использование: @cache_decorator (без скобок)
    func = maxsize
    maxsize = None
    return decorator(func)

# Использование: @cache_decorator() или @cache_decorator(maxsize=10)
return decorator
```
## 3. Программа
```python
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if maxsize is not None and len(memory) > maxsize:
                ...
                if logging_enabled:
                    print(
                        f"{indent}[{time.strftime('%H:%M:%S')}] ⚠️ Превышен лимит кэша ({maxsize}), удалён {oldest_key}")
...
# ============== ВАША ПЕРВАЯ ПРОГРАММА (Фибоначчи) ==============
print("=" * 60)
...
# ============== ВАША ВТОРАЯ ПРОГРАММА (с декоратором) ==============
print("\n" + "=" * 60)
...
print("\n" + "=" * 60)
print("ЗАДАЧА 6: Исправленный вариант вашего кода")
print("=" * 60)
